...
def chat_session(prompt: str, model: str, history: Optional[List[Dict[str, str]]] = None) -> Tuple[str, str]:
    """Execute a chat session with conversation history.
...
def get_consensus_from_models(prompt: str, models: List[str]) -> str:
    ...
    if len(models) == 1:
        ...
        return _get_single_model_response(prompt, models[0])
    
    # Multiple models - gather responses and synthesize
    responses = []
    
    # Collect responses from each model
    for i, model in enumerate(models):
        try:
            logger.info(f"Getting response from model {i+1}/{len(models)}: {model}")
            
            # Build context with previous responses
            context = _build_consensus_context(prompt, responses)
            
            response = _get_single_model_response(context, model)
            responses.append({
                "model": model,
                "response": response
            })
